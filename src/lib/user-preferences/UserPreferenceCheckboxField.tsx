...
const UserPreferenceCheckboxField /*: React.FC<UserPreferenceCheckboxFieldProps>*/ = ({
  ...
  const onChange = (checked: boolean) => {
    const checkedValue: UserPreferenceCheckboxFieldValue = checked
      ? trueValue
      : falseValue;
    checkedValue !== currentUserPreferenceValue &&
      setCurrentUserPreferenceValue(checkedValue);
  };
  ...
  return loaded ? (
    ...
    <Skeleton
      height="30px"
      width="100%"
      data-test={`dropdown skeleton ${id}`}
    />
  );
};
